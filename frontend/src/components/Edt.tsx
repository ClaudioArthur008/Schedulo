...
const Edt = () => {
  ...
  useEffect(() => {
    if (userInfo?.enseignant?.id_enseignant) {
      api.get(`/disponibilite/this-week/${userInfo.enseignant.id_enseignant}`)
        .then(res => {
  ...
  const exportDisponibilites = async () => {
    ...
    const existingDispos = await api.get(
      `/disponibilite/specific-week/${userInfo.enseignant.id_enseignant}`
    );
    ...
    for (const slot of availableSlots) {
      ...
      console.log("Exporting slot:", slot.date);

      await api.post('/disponibilite', {
